...
- do not introduce pre-commit as part of a starter harness unless the human
  wants that workflow,
- prefer wiring existing checks into `scripts/verify.sh` first,
...
- let human local commits and agent workflows share the same verification
  contract,
- call `scripts/verify.sh` or narrower shared scripts instead of duplicating
  command lists,
...
- whether `scripts/verify.sh` is fast enough for commit-time use,
- which checks belong in pre-commit rather than CI or manual verification.

Potential future assets:

- pre-commit config snippets that call `scripts/verify.sh`,

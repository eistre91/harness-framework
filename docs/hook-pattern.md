...
Use when: a target repo needs hooks beyond the narrow Bounded Work
`repo-checks-on-stop` adapter, or when the installer needs the full hook adapter
pattern as a reference. Do not read this for ordinary Bounded Work installation
unless `docs/install/bounded-work.md` or the human-approved scope routes here.
...
The Bounded Work Stop adapter is intentionally narrower than this layout. It uses
